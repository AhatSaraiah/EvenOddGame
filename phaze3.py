...
def round_boss(player1_name, boss):
    '''
    Simulate a boss round
    Args:
    - player1_name (str): Name of player 1.
    - boss (int): Score of the boss.
    Returns:- tuple: Random number, updated score of player 1, updated score of the boss.'''
    rand = random.randint(-5, 13)
    if rand % 2 == 0 and rand < 0:
        print("Negative number")
    elif rand % 2 == 0 and rand > 0:
        players[player1_name] += 1
    else:
        players[boss] += 1
    return rand, players[player1_name], players[boss]
...
def even_odd_game(i, rounds, players):
    '''Simulate the entire game, including regular and boss rounds.
    ...
    while rounds not in players.values():
        ...
        if rounds > 5:  # Check if it's time for the boss round
            players['boss'] = 0
            while players['boss'] < 3:
                new_round, points1, boss = round_boss(winner, boss)
                print_status(i, winner, "Boss", new_round, points1, boss, winner)
                i += 1
            break
